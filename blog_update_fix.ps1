...
$menuPattern = [regex] '<li\s+class="menu-item-has-children"\s*>\s*<a href="#">Blog</a>\s*<ul class="sub-menu">\s*<li>\s*<a href="blog.html">Blog List</a>\s*</li>\s*<li>\s*<a href="blog-2.html">Blog Grid</a>\s*</li>\s*<li>\s*<a href="blog-details.html">Insight Details</a>\s*</li>\s*</ul>\s*</li>'
...
$articleData = @{
    2 = @{ title = 'Delivering Climate Resilience Through Local Partnerships'; summary = 'A practical look at how community partnerships and institutional support build resilient systems for climate adaptation.'; top = 'EquiRise helps institutions and communities build resilient systems for climate adaptation through partnership, planning, and operational support.' }
    3 = @{ title = 'Transforming Procurement and Supply Chain Governance'; summary = 'Insights on building transparent procurement systems that improve accountability and value across public and private sectors.'; top = 'EquiRise helps government and business systems become more transparent, efficient, and resilient through better procurement governance.' }
    4 = @{ title = 'Building Capacity for Sustainable Growth'; summary = 'How targeted learning and institutional capacity-building drive institutional change and lasting outcomes.'; top = 'EquiRise develops practical capacity-building programs that strengthen institutional performance and sustainable growth.' }
}

foreach ($i in 2..4) {
    $out = [regex]::Replace($template, 'href="blog-details\.html"', 'href="blog-details-' + $i + '.html"')
    $out = [regex]::Replace($out, '<h4 class="box-top-title pe-xl-5">.*?</h4>', '<h4 class="box-top-title pe-xl-5">' + $articleData[$i].top + '</h4>', [System.Text.RegularExpressions.RegexOptions]::Singleline)
    $out = $out -replace '<h3 class="title mb-3">What EquiRise Consulting Offers</h3>', '<h3 class="title mb-3">' + $articleData[$i].title + '</h3>'
    $out = [regex]::Replace($out, '<p class="mb-50">.*?</p>', '<p class="mb-50">' + $articleData[$i].summary + '</p>', [System.Text.RegularExpressions.RegexOptions]::Singleline, 1)
    Set-Content -Path (Join-Path $root ('blog-details-' + $i + '.html')) -Value $out -Encoding UTF8
    Write-Host "Created blog-details-$i.html"
}
